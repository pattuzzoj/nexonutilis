...
export default function Search() {
  ...
  function filterList(e: any) {
    ...
    if(e.target.value != '') {
      ...
      data.map.forEach((item: any) => {
        if(item.url.lastIndexOf('/')) {
          if(item.type == "resources" && item.title.toLowerCase().startsWith(e.target.value.toLowerCase())) {
  ...
  return (
    <div class="relative">
      <input class="peer w-full bg-gray-500 p-2 rounded-lg" type="text" onInput={filterList} value={value()} placeholder="Search Resources" />
