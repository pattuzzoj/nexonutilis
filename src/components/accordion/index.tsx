  ...
  return (
    <nav class="w-full flex flex-col gap-2" aria-labelledby={props.data.title}>
      <Title as={(props.data.url.substring(1).split("/").length + 1) <= 6 ? (props.data.url.split("/").length) : "6"} id={props.data.title}>
        <a class={`${isOpen() ? "text-white bg-gray-500 flex-row-reverse" : "text-gray-900 dark:text-white"} group flex items-center gap-2 p-2 rounded-lg font-medium text-base hover:text-white hover:bg-gray-400 hover:scale-95 group`} href={props.data.url} onClick={(e) => {
          e.preventDefault();
          setIsOpen(!isOpen());
          navigate(isOpen() ? props.data.url : props.data.url.slice(0, (props.data.url).lastIndexOf("/")));
        }}>
          <span class="group-hover:animate-spin">
