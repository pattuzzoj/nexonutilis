  ...
  return (
    ...
    >
      <Show when={props.favorite}>
        <a class="flex justify-between items-center rounded-xl p-2 text-sm hover:bg-gray-100 hover:bg-hover" href={props.url}>{props.category}</a>
      </Show>
      <span class="flex justify-between">
        ...
        <span class="flex items-center text-black dark:text-white">
          <Show when={favoriteData().find((savedItem: any) => savedItem.id == props.id)} fallback={(
            <button class="group rounded-xl p-2 hover:scale-90 hover:text-hover hover:bg-hover" onClick={() => {favoriteStore.add({...props}); favoriteStore.getAll();}}>
            ...
            <button class="group rounded-xl p-2 hover:scale-90 hover:text-hover hover:bg-hover" onClick={() => {favoriteStore.del(props.id); favoriteStore.getAll()}}>
          ...
          <button class="rounded-xl p-2 hover:scale-90 hover:text-hover hover:bg-hover" onClick={() => copy(props.url)}>
      ...
      <span class="flex flex-col md:flex-row gap-2 md:gap-4 w-full">
        ...
        shadow-sm shadow-blue-200/20 dark:hover:shadow-blue-200 hover:text-hover bg-primary hover:bg-hover hover:scale-90" target="_blank" href={props.url}>Access <Icon name="OcLinkexternal2" class="group-hover:scale-110 size-4"/></a>
      </span>
    </div>
  );
}
