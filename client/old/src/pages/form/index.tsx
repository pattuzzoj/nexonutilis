  ...
  return (
    <Main class="relative">
            ...
            {(item) => (
              <span class="relative flex justify-between items-center gap-4 border-t-[1px] even:bg-gray-500 py-2 px-1 pl-4">
                <Show when={item.hasOwnProperty("items")}>
                  <button class="absolute top-50 left-1.5 rounded text-lg bg-gray-900" onClick={() => {setList(item?.items); setPath([...path(), {title: item.title, items: item?.items}])}}></button>
                </Show>
                ...
                <Show when={item.hasOwnProperty("type")}>
                  <button class="text-lg" onClick={() => {setType(item.type); setData({...initialValue, parent_category_id: item.id, category_id: item.id}); setTypeMenu("create"); setMenu(true);}}></button>
                </Show>
                  <button class="text-lg" onClick={() => {setData({...item, url: item.url.substring(item.url.lastIndexOf('/'))}); setTypeMenu("edit"); setMenu(true);}}></button>
      ...
      <Show when={menu()}>
        ...
        ">
          <form class="relative flex flex-col gap-2 p-6">
            <button class="absolute top-0 right-0 p-2" onClick={() => setMenu(false)}></button>
